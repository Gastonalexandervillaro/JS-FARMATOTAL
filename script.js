...
const imageList = [
  ...
  function filterImages() {
    ...
    xhr.open("GET", "data.json", true); // Cargar el archivo JSON
    xhr.onload = function () {
        if (xhr.status === 200) { // Verificar que la carga sea exitosa
            const imageList = JSON.parse(xhr.responseText); // Convertir el JSON en un array
